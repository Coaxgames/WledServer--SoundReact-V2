...
namespace WledSRServer.Audio
{
    internal static class AudioCaptureManager
    {
        ...
        public static float[] FFTBandGains { get; } = Enumerable.Repeat(1f, 16).ToArray();
        public static byte[] FFTBandOriginalValues { get; } = new byte[16];
        public static int FFTBandNotches { get; set; } = 10;
        public static AudioProcessChain? ActiveChain;

        public static void LoadFFTBandGains()
        {
            var values = Properties.Settings.Default.FFTBandGains.Split(',');
            for (var i = 0; i < FFTBandGains.Length && i < values.Length; i++)
                if (float.TryParse(values[i], System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var gain))
                    FFTBandGains[i] = Math.Clamp(gain, 0, 2);
        }

        public static void SaveFFTBandGains()
        {
            Properties.Settings.Default.FFTBandGains = string.Join(",", FFTBandGains.Select(g => g.ToString("R", System.Globalization.CultureInfo.InvariantCulture)));
            Properties.Settings.Default.Save();
        }

        public static void NormalizeFFTBandGains()
        {
            if (FFTBandNotches == 0)
                return;

            for (var i = 0; i < FFTBandGains.Length; i++)
            {
                var notch = (int)Math.Round(FFTBandGains[i] * FFTBandNotches / 2f);
                FFTBandGains[i] = notch * 2f / FFTBandNotches;
            }
        }
