...
namespace WledSRServer.Audio.AudioProcessor.Packet
{
    internal class SetPacket : Processor
    {
        ...
        public override bool Process()
        {
            ...
            var autoGainLowerLimit = Math.Clamp(Properties.Settings.Default.AutoGainLowerLimit, 0, 255);
            var autoGainUpperLimit = Math.Clamp(Properties.Settings.Default.AutoGainUpperLimit, autoGainLowerLimit, 255);

            for (var bucket = 0; bucket < _buckets.Values.Length; bucket++)
            {
                AudioCaptureManager.FFTBandOriginalValues[bucket] = (byte)Math.Clamp((_buckets.OriginalValues[bucket] + _agc.Offset) / _agc.Span * 255, 0, 255);
                var value = (byte)Math.Clamp((_buckets.Values[bucket].Value + _agc.Offset) / _agc.Span * 255, 0, 255);
                if (!Properties.Settings.Default.ManualGain)
                {
                    if (value < autoGainLowerLimit)
                        value = 0;
                    value = Math.Min(value, (byte)autoGainUpperLimit);
                }
                _packet.FFT_Bins[bucket] = value;
            }
